...
def get_air_quality_data(api_key, limit):
    ...
    try:
        ...
        logging.info(f'Response Text: {response.text[:200]}')  # Log first 200 characters of the response
        ...
        if response.status_code == 200:
            try:
                ...
                # If json_data is empty, log it and raise an error
                if not json_data:
                    logging.error('Received empty JSON data')
                    sys.exit(1)

                # Proceed with saving the JSON to a file
                logging.info('Writing the JSON file into local location before it moved to snowflake stage')
                with open(file_name, 'w') as json_file:
                    json.dump(json_data, json_file, indent=2)
                ...
                stg_location = '@dev_db.stage_sch.raw_stg/'+today_string+'/'
                sf_session = snowpark_basic_auth()
                ...
                logging.error('Failed to parse JSON from the response')
                sys.exit(1)
...
api_key = '579b464db66ec23bdd000001d047f667062f40164153b50f55fb46c5'
